...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc,cur) => acc += cur.filter(num => num === target).length ,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(num => typeof num !== 'string' && num % 5 === 0).map(num2 => Math.pow(2,num2)));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((first,second) => first = (parseInt(first.height) < parseInt(second.height)? first: second)).name;
};
