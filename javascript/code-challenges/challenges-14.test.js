...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(word => word[0] = word[0].toUpperCase() + word.slice(1));
};
...
------------------------------------------------------------------------------------------------ */
const isWin = (permutation,char) => {
  return permutation.filter(cell => cell===char).length === permutation.length;
};

//each array in board represents a row
const checkHorizontals =(arr,char) => {
  let win = false;
  for(let i = 0; i < 3; i++) {
    if (isWin(arr[i],char)) {
      win = true;
...
const checkVertical = (arr,col,char) => {
  let permutation = [];
  for(let i = 0; i < 3; i++) {
    permutation.push(arr[i][col]);
  }
  return isWin(permutation,char);
};

const checkVerticals = (arr,char) => {
  let win = false;
  for(let i = 0; i < 3; i++) {
    if(checkVertical(arr,i,char)) {
      win = true;
    }
  }
  return win;
...
const checkLeftDiag = (arr,char) => {
  let permutation = [];
  for(let i = 0; i < 3; i++) {
    for(let j = 0; j < 3; j++) {
      if(i===j) {
        permutation.push(arr[i][j]);
      }
    }
  }
  return isWin(permutation,char);
...
const checkDiags = (arr,char) => {
  //diags must contain center cell
  if(arr[1][1] !== char) {
    return false;
  } 
  if(checkLeftDiag(arr,char)) {
    return true;
  }
  if(checkRightDiag(arr,char)) {
    return true;
  }
  return false;
};
...
const checkChar =(arr,char) => {
  let win = false;
  if(checkDiags(arr,char)) {
    win = true;
  }
  if(checkHorizontals(arr,char)) {
    win = true;
  }
  if (checkVerticals(arr,char)) {
    win = true;
  }
  return win;
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  // both diagnol win conditions and two vertical require center square, 
